...
HRESULT Renderer::InitDevice()
{
	...
	VideoConfig& cfg = _emu->GetSettings()->GetVideoConfig();
	bool flipSwapSupported = false;
	bool enableFlipSwap = cfg.EnableVariableRefreshRate && _fullscreen == FullscreenMode::Borderless;
	bool allowTearing = false;

	ComPtr<IDXGIFactory5> factory5;
	if(SUCCEEDED(factory->QueryInterface(IID_PPV_ARGS(&factory5)))) {
		flipSwapSupported = false;
		if(enableFlipSwap) {
	...
	_enableFlipSwap = enableFlipSwap;
	_allowTearing = allowTearing;
	_bufferCount = _enableFlipSwap ? 2 : 1;
	...
	sd.SwapEffect = _enableFlipSwap ? DXGI_SWAP_EFFECT_FLIP_DISCARD : DXGI_SWAP_EFFECT_DISCARD;
