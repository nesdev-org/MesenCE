	...
	public class DefaultLabelHelper
	{
		...
		private static void SetDefaultNesLabels()
		{
			...
			if(EmuApi.GetRomInfo().Format == RomFormat.Fds) {
				...
				LabelManager.SetLabel(0x09D3, MemoryType.NesPrgRom, "CounterLogic", "Input: A, Y = end zero-page address of counters, X = start zero-page address of counters " + Environment.NewLine + "Output: " + Environment.NewLine + "Affects: A, X, $00 " + Environment.NewLine + "Desc: This decrements several counters in zero-page. The first counter, at X, is a decimal counter 9 -> 8 -> 7 -> ... -> 1 -> 0 -> 9 -> ... Counters 1...A are simply decremented and stay at 0. Counters A+1...Y are decremented when the first counter does a 0 -> 9 transition, and stay at 0.");
