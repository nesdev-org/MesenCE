...
{
	private ELF<uint> elf;

	private Section<uint>[] sectionRegions;
	private MemoryRegionFinder sectionRegionFinder;

	/// <param name="path">The image path.</param>
	/// <param name="loadOffset">Offset from where image was loaded.</param>
	public ElfImage(string path, ulong loadOffset = 0)
	{
		elf = ELFReader.Load<uint>(path);
		...
		SymbolTable<uint>? symbols = elf.Sections.FirstOrDefault(s => s.Type == SectionType.SymbolTable) as SymbolTable<uint>;

		if(symbols == null || !symbols.Entries.Any()) {
			symbols = elf.Sections.FirstOrDefault(s => s.Type == SectionType.DynamicSymbolTable) as SymbolTable<uint>;
		}

		if(symbols != null) {
			PublicSymbols = symbols.Entries;
		} else {
			PublicSymbols = new List<SymbolEntry<uint>>();
		}

		sectionRegions = elf.Sections.Where(s => s.LoadAddress > 0).OrderBy(s => s.LoadAddress).ToArray();
		sectionRegionFinder = new MemoryRegionFinder(sectionRegions.Select(s => new MemoryRegion { BaseAddress = s.LoadAddress, RegionSize = s.Size }).ToArray());
	}

	/// <summary>
	/// Gets the public symbols.
	/// </summary>
	public IEnumerable<SymbolEntry<uint>> PublicSymbols { get; private set; }
	...
	{
		foreach(Section<uint> section in elf.Sections) {
			if(section.Name == sectionName) {
