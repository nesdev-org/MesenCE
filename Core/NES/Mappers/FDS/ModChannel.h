...
{
private:
	const int16_t _modReset = 0xFF;
	const int16_t _modLut[8] = { 0, 1, 2, 4, _modReset, -4, -2, -1 };
...
public:
	...
	bool TickModulator(bool haltWaveform)
	{
		...
		if(_frequency > 0 && !haltWaveform && ++_modM2Counter == 16) {
			...
			if((_modAccumulator & 0xFFF) < _frequency || _forceCarryOut) {
				...
				UpdateCounter(offset == _modReset ? 0 : _counter + offset);
			}
	...
	{
		int16_t offset = _modLut[_modTable[_modTablePosition]];
		return offset == _modReset ? 0x0C : offset & 0x0F;
	}
};
